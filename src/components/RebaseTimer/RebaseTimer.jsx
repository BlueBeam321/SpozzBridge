...
function RebaseTimer() {
  ...
  function initializeTimer() {
    console.log("[tz]==> initializetimer, secondsToEpoch: ", secondsToEpoch);
  ...
  useEffect(() => {
    ...
    if (secondsToRefresh > 0) {
      ...
      if (secondsToRebase < 0) {
        ...
        reload();
        setRebaseString("");
        console.log("[tz]==> secondsToRebase < 0 : ", secondsToRebase);
      } else {
        ...
        {currentBlock ? (
          secondsToRebase > 0 ? (
            <>
              ...
              <Trans>to next Epoch</Trans>
            </>
          ) : (
            <strong>Distributing Rewards</strong>
          )
        ) : (
